...
The restart command is the one you should use to restart the containers without removing the containers.

```{warning}
Running restart will more than likely make you loose the files of containers that run without volumes, its recommended to run a backup always.
```
